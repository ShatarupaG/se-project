...
async function calculateDecisionStyle() {
  try {
    ...
    const response = await fetch(`http://34.125.80.10:3000/getUserResponses?userId=${user._id}`);
...
async function fetchDecisionDetails(decisionStyle) {
  try {
    const response = await fetch(`http://34.125.80.10:3000/getDecisionDetails?decisionStyle=${decisionStyle}`);
...
async function saveAttemptResult(userId, decisionStyle) {
  try {
    const response = await fetch('http://34.125.80.10:3000/saveAttemptResult', {
      method: 'POST',
      headers: {
